...
import PolishingImg from "../../../../assets/polishing.webp"
import NanoCeramicImg from "../../../../assets/nano-ceramic.webp"
import ThermalInsulationImg from "../../../../assets/thermal-insulation.webp"
...
export default function ServicesSection() { 
  ...
  return (
    <section className="relative py-24 services-section-bg">
      ...
      <div className="w-11/12 mx-auto flex flex-col gap-15 items-center">
        ...
        <div className={`w-full h-fit flex gap-y-20 ${!isEnglish && 'flex-row-reverse'} ${isMobile ? 'gap-20' : 'gap-x-10'} ${isSmallMobile ? 'px-8' : 'px-15'} ${isMedium && 'flex-wrap'} justify-center items-center mt-10`}>
          {services.map((service, serviceIndex) => (
            ...
            >
              <div className={`${isEnglish ? 'skew-x-25' : '-skew-x-25'} relative flex flex-col w-auto min-w-[200px] max-w-[500px] text-center`}>
                <div className={`absolute z-2 ${isEnglish ? 'left-0' : 'right-0'} -top-10 p-4 rounded-lg flex items-center justify-center bg-primary w-fit h-[50px] shadow-[0_5px_20px_0px_rgba(0,0,0,8)]`}>
                  <h3 className={`text-lg lg:text-xl font-semibold leading-5 ${isEnglish ? '-skew-x-25' : 'skew-x-25'}`}>{service.title}</h3>
                </div>
                <div className="rounded-2xl overflow-hidden shadow-[0_0_20px_5px_rgba(0,0,0,8)]">
                  <img
                    src={servicesImg[serviceIndex]}
                    alt={service.title}
                    loading="lazy"
                    className={`w-full scale-155 md:scale-140 object-cover ${isEnglish ? '-skew-x-25' : 'skew-x-25'} shadow-lg mb-10`}
                  />
                </div>
              </div>
            </motion.div>
        ...
        {products.map((product, productIndex) => (
          ...
          >
            ...
            className={`h-full ${isMobile ? 'w-full' : 'w-[280px]'} relative cursor-pointer transition-all duration-300 hover:scale-110 hover:z-10 flex items-center justify-center px-15 py-5 rounded-2xl overflow-hidden ${isEnglish ? '-skew-x-25' : 'skew-x-25'} shadow-[0_0_20px_5px_rgba(0,0,0,8)]`}
            >
              <img
                src={product.logo}
                alt=""
